...
def dashboard():
    ...
    return render_template('dashboard.html', medicines=medicines, today=today, seven_days_later=seven_days_later)
...
def add_item():
    ...
    count = Medicine.query.filter_by(location=location).count()
    if count + quantity > 4:
        flash(f"Cannot add {quantity} item(s). Rack at location {location} can only hold {4 - count} more item(s).",
              "danger")
        return redirect(url_for('dashboard'))
